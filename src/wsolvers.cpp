...
pair<double, double> get_crit(double nucleon_mass,double saturation_density, double binding_energy, double degeneracy, vector<double> scalar_exp, vector<double> vec_exp, bool print, double crit_T, double crit_n, vector<double> scalar_coeff, vector<double> vec_coeff){
  ...
  if(scalar_coeff.size()!=0&&abs(scalar_coeff[0])<1e-10&&vec_coeff.size()!=0&&abs(vec_coeff[0])<1e-10){
...
double press_dn_solv(double degeneracy, double nucleon_mass,double number_density, vector<double> scalar_coeff,vector<double> scalar_exp, vector<double> vec_coeff, vector<double> vec_exp){
  pair<double, double> x=get_mass_eff_scalar_density(degeneracy, nucleon_mass, scalar_coeff, scalar_exp, number_density, false);
  return press_dn(degeneracy, x.first,number_density,scalar_coeff, scalar_exp, vec_coeff, vec_exp,x.second);
}
...
bool validate_interaction(double error, vector<double> coeff_guess, vector<double> exp_guess,vector<double> coeff, vector<double> exp, string filename, int timestamp, void* params){
  ...
  cout<<CP.first<<" "<<CP.second/saturation_density<<endl;
  if(abs(round_to_n_digits(CP.first,3)-round_to_n_digits(critical_temperature,3))>eps||abs(round_to_n_digits( CP.second/saturation_density,2)-round_to_n_digits( critical_density/saturation_density,2))>eps){
    return false;
  }
        ...
        for(double exp1_guess=boundaries[2][0]; exp1_guess<=boundaries[2][1]; exp1_guess+=boundaries[2][2] ){
          for(double exp2_guess=boundaries[3][0]; exp2_guess<=boundaries[3][1]; exp2_guess+=boundaries[3][2] ){
            ...
            try{
              ...
              if(get<4>(result)<2e-5&&(count<num_sol||num_sol<=0)){

                double coeff1_cut=round_to_n_digits((get<2>(result)),3);
                double coeff2_cut=round_to_n_digits((get<3>(result)),3);
                double exp1_cut=round_to_n_digits((get<0>(result)),3);
                double exp2_cut=round_to_n_digits((get<1>(result)),3);
                bool found=false;
